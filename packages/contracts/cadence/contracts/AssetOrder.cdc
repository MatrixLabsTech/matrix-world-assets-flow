import AssetFee from "AssetFee.cdc"
...
pub contract AssetOrder {
    ...
    pub fun addOrder(
        ...
        let addPayment = fun (type: String, address: Address, rate: UFix64) {
            ...
            percentage = percentage - (type == AssetOrder.BUYER_FEE ? 0.0 : rate)
            assert(rate >= 0.0 && rate < 1.0, message: "Sum of payouts must be in range [0..1)")
        }

        addPayment(AssetOrder.BUYER_FEE, AssetFee.feeAddress(), AssetFee.buyerFee)
        addPayment(AssetOrder.SELLER_FEE, AssetFee.feeAddress(), AssetFee.sellerFee)

        for cut in extraCuts {
            addPayment(AssetOrder.OTHER, cut.address, cut.rate)
        }

        for royalty in royalties {
            addPayment(AssetOrder.ROYALTY, royalty.address, royalty.rate)
        }

        addPayment(AssetOrder.REWARD, orderAddress, percentage)
